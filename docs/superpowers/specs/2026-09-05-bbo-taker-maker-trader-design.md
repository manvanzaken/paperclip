...
- `RateBudget`: token buckets per venue from the registry — documented limits MEXC 20 orders/2 s and 20 cancels/2 s, BloFin 30 trading requests/10 s (all trading endpoints share it), configured with ~10% headroom (18/2 s, 27/10 s) because our window is measured at send time and the venue's at receive time. A 429 penalty halves capacity for entries and requotes only; hedges, closes and cancels keep the full window. `reserve` tokens (MEXC 4, BloFin 6) are only spendable by hedges, closes, flattens, and cancels, so new entries and requotes can never starve a risk-reducing call. `MAX_RESTING_MAKERS_PER_VENUE=2` keeps requote demand inside the tightest budget.
